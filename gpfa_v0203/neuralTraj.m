...
%        John Cunningham  jcunnin@stanford.edu
  ...
  binWidth      = 20; % in msec
  numFolds      = 0;
  foldMethod    = 'rand'; % Options are rand (randomly select trials or seq
  xDim          = 8;
  extraOpts     = assignopts(who, varargin);
  useSqrt       = true;    % logical specifying whether or not to use square-root transform
%               on spike counts (default: true)
  ...
  seq  = getSeq(dat, binWidth, extraOpts{:},'useSqrt',useSqrt);
  if isempty(seq)
  ...
  for cvf = 0:numFolds
    ...
    %fname = []; % Empty because this variable is passed to the GPFA engine
  ...
  % Note (ADD 2016/09/02): Saving has been disabled, as the saved structure
  % is not the same as the results structure used in subsequent analyses.
  end
