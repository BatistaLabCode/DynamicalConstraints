...
% Aug 21, 2011: Added option for causal smoothing
  ...
  elseif ismember(ws.method, {'gpfa'}) & causal
      switch causalMode
          case 'full'   % Standard causal GPFA
            seqNew = causalInference(seqNew, ws.estParams, extraOpts{:});
          case 'rt'     % Truncated real-time implementation
            seqNew = causalInferenceRT(seqNew, ws.estParams, extraOpts{:});
      end
    C              = ws.estParams.C;
    X              = [seqNew.xfi];
    [Xorth, Corth] = orthogonalize(X, C);
