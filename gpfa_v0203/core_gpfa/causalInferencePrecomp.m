...
% Adjust time constants.  First convert the 'gamma' values to time
% constants, find average, and convert back to 'gamma'.  Note: this is
% currently disabled, as it is shared code.  Eventually we might want to
% set up this function such that the scaling is controlled by an optional
% input.
if fixTau
    tau = params.binWidth ./ sqrt(params.gamma);
    tau_mean = mean(tau);
    tau = ones(size(tau)) * tau_mean;
    gamma = (params.binWidth./tau).^2;
    params.gamma = gamma;
end
